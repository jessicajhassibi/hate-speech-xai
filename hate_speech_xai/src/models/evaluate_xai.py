...
def evaluate_xai_on_dataset(dataset: Dataset, method_name: str, method: Callable, max_samples: int | None = None) -> dict:
	"""Evaluates the passed XAI method on a dataset and returns per-metric averages."""
	top_k_scores = []
	samples = dataset if max_samples is None else dataset.select(range(max_samples))

	for i in range(len(samples)):
		post = samples[i]
		label = get_majority_label(post["annotators"]["label"])
		if label == 1: # Skip normal posts (uniform rationale, not meaningful to evaluate)
			continue
